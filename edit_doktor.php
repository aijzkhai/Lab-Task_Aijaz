...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['Specialization'])) {
        ...
        $specialization = mysqli_real_escape_string($connect, trim($_POST['Specialization']));
    }
    ...
    if (empty($error)) {
        $q = "SELECT ID FROM doktor WHERE Specialization ='$specialization' AND ID != $id";
        $result = @mysqli_query($connect, $q);
        
        if (mysqli_num_rows($result) == 0) {
            $q = "UPDATE doktor SET FirstName='$firstname', LastName='$lastname', Specialization='$specialization', Password='$password' WHERE ID='$id' LIMIT 1";
            $result = @mysqli_query($connect, $q);
            ...
            echo '<p class="error">The Specialization is already assigned to another doctor.</p>';
        }
    } else {
